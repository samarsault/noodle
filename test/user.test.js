...
describe("User Service", function () {
	...
	it('shows updated info of a user', async (done) => {
		...
		const updated = await userService.updateInfo(data.student._id, updateObject)
	...
	it('checks wether it update access or not', async (done) => {
		...
		// For instructor
		const updateAccessIns = await userService.updateAccess(data.student._id, 'instructor',data.course.name);
		let user = await User.findOne({_id: data.student._id})
		const course = await Course.findOne({_id: data.course._id})
		expect(user.role).toBe('instructor')
		expect(course.instructors).toEqual(expect.arrayContaining([data.student._id]))

		// For admin
		const updateAccessAdmin = await userService.updateAccess(data.student._id, 'admin')
	...
	it('Gets the data of a user successfully', async (done) =>{
		...
		expect(name(search)).toEqual(expect.arrayContaining([data.student.name, dummy2.name]));
		//Not more than 5
	...
	it('paginate the search result of users', async (done) => {
		...
		expect(name(paginatedSearch.docs)).toEqual(expect.arrayContaining([data.student.name, dummy2.name]));
