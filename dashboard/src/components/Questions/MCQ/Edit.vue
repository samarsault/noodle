	...
	<div>
		...
		<ol type="A">
			<li v-bind:key="index" v-for="(option, index) in local.options">
				<div class="separated-elements">
				<input style="display: inline-block" type="text" @input="updateOption(index, $event.target.value)" :value="option"/>
		...
		<div v-if="local.options.length > 0" style="margin-top:10px">
			...
			<select @input="update('answer', parseInt($event.target.value))" :value="local.answer">
				<option v-bind:key="index" v-for="(option, index) in local.options" :value="index">
...
export default {
	...
	addOption() {
	    this.$emit('input', {
		...this.local,
		options: [...this.local.options, `Option ${this.local.options.length + 1}`]
	    });
	},
	removeOption(index) {
	    const newOptions = [...this.local.options];
