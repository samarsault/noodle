				...
				<tbody>
					<tr v-bind:key="index" v-for="(question, index) in questions">
						...
						<td>
							...
							<button class="small error" @click="deleteQuestion(index)"><Bin /></button>
						</td>
					</tr>
				</tbody>
			</table>
...
export default {
  ...
  data() {
    return {
			showEditor: false,
			showSelect: false,
			course_id: '5eb40a8f88c41e30eabb8eda',
			presetIndex: -1,
			preset: {},
			questions: [],
			questionTypes: [
	...
	mounted() {
		axios.get(`/api/courses/${this.course_id}/questions`)
			.then( ({data}) => {
				this.questions = data;
			}
		)
	...
	methods: {
		...
		deleteQuestion(index) {
			...
			axios.delete(`/admin/courses/${this.course_id}/questions/${question._id}`)
			.then(({ data }) => {
				if (data.ok) {
					alert('Item deleted')
