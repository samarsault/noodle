<template>
	<div class="container">
		...
		<div v-if="quiz">
			...
			<button class="outline icon-button" @click="$emit('close')"><Back /><span class="icon-left">Back</span></button>
			<button class="primary icon-button" @click="showEditor = true"><Plus decorative/><span class="icon-left">Add Question</span></button>
			</div>
			<table v-if="quiz.questions.length > 0">
				...
				<tbody>
					<tr v-bind:key="index" v-for="(question, index) in quiz.questions">
						...
						<td>
							<button class="small outline" @click="editQuestion(index)"><Edit /></button>
							<button class="small error" @click="deleteQuestion(question, index)"><Bin /></button>
						</td>
					</tr>
				</tbody>
			</table>
...
export default {
  ...
  data() {
	...
	methods: {
		getQuizData() {
			axios.get(`/api/courses/${this.course_id}/quiz/${this.quizId}`)
				.then( ({data}) => {
					this.quiz = data;
				})
		},
		...
		deleteQuestion(question, index) {
			axios.post(`/admin/courses/${this.course_id}/quiz/update`, {
				type: 'delete',
				quiz_id: this.quizId,
